...
#theorem(
  ...
  + $forall lambda a_(1), lambda a_(2) in RR ==>$ функция $lambda a_(1) f + lambda a_(2) g$ интегрируема в несобственном смысле и $ limits(integral)_(a)^(omega) lr((lambda a_(1) f(x) + lambda a_(2) g(x)), size: #125%) d x = lambda a_(1) limits(integral)_(a)^(omega) f(x) d x + lambda a_(2) limits(integral)_(a)^(omega) g(x) d x. $

  + $c in [a, omega) ==>$ $ limits(integral)_(a)^(omega) f(x) d x = limits(integral)_(a)^(c) f(x) d x + limits(integral)_(c)^(omega) f(x) d x. $

  + Функция $phi$ непрерывно дифференцируема и строго монотонна на $[alpha, gamma), #h(6pt) phi(alpha) = a$ и $phi(beta) -> omega$ при $beta -> gamma, beta in [alpha, gamma)$. Тогда несобственный интеграл от $(f compose phi)phi'$ на $[alpha, gamma)$ существует и $ limits(integral)_(a)^(omega) f(x) d x = limits(integral)_(alpha)^(gamma) f(phi(t))phi'(t) d t. $
][
...
#theorem(
  ...
  Несобсвенный интеграл $limits(integral)_(a)^(omega) f(x)d x$ сходится тогда и только тогда, когда
