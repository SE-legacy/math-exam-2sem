...
#definition()[
  ...
  $ (partial f)/(partial x_1) (overline(x)_0), quad "или", quad f'_(x_1) (overline(x)_0). $
...
#definition()[
  ...
  #equ(
    $
      f(overline(x)) - f(overline(x)_0) = & A_1 (x_1-x_1^0) + A_2 (x_2 - x_2^0) + dots + A_m (x_m - x_m^0) + \
  ...
  #equ(
    $
      f(overline(x)) - f(overline(x)_0) = A_1 (x_1-x_1^0) + A_2 (x_2 - x_2^0) + dots + A_m (x_m - x_m^0) + o(abs(overline(x) - overline(x)_0)).
    $,
    id: <eq:diff_cond3>,
  )
...
#theorem(name: "Теорема (о существовании частной производной у дифференцируемой функции)")[
  ...
  $ (partial f)/(partial x_k) (overline(x)_0) = A_k, $
  ...
  $ (partial f)/(partial x_1) (overline(x)_0) = A_1. $
