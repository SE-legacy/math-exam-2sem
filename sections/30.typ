...
#definition()[
  Отображение вида $overline(f) : X -> RR^k$, где $X subset RR^m, #h(6pt) m > 1, #h(6pt) k > 1$, называют вектор функцией векторного аргумента.
]
...
#definition(name: "Определение (предела по Гейне)")[
  Пусть $overline(x)_0$ --- предельная точка области определения функции $overline(f) : X -> RR^k, #h(6pt) overline(A) in RR^k$. Вектор $overline(A)$ называют пределом функции $overline(f)$ в точке $overline(x)_0$, если для любой последовательности $(overline(x)_n)$ точек, принадлежащих $D(overline(f))$, удовлетворяющей условиям: $overline(x)_n != overline(x)_0, #h(6pt) overline(x)_n -> overline(x)_0$, имеет место
...
#definition(name: "Определение (непрерывности по Гейне)")[
  Функция $overline(f)$ называется непрерывной в точке $overline(x)_0$, если для любой последовательности $(overline(x)_n)$ точек, принадлежащих $D(overline(f))$, удовлетворяющей условию $overline(x)_n -> overline(x)_0$, имеет место
