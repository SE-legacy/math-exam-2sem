...
#theorem()[
  Определение предела по Коши $<==>$ определение предела по Гейне.
][]
...
#theorem()[
  Определение непрерывности по Коши $<==>$ определение непрерывности по Гейне.
][]

#theorem()[
  Функция $overline(f) = (f_1, dots, f_k)$ непрерывна в точке $overline(x)_0$ $<==> forall i = 1, dots, k$ функции $f_i$ непрерывны в ней.
][]
