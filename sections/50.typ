...
Тогда функция $phi'$ сохраняет знак на отрезке $[a_1, b_1]$, и функция $phi$ является возрастающей, если $phi'(u) > 0$, и убывающей, если $phi'(u) < 0$. Согласно правилу замены переменной в интеграле Римана имеем
...
Обозначим $overline(gamma)^* (u) = (overline(gamma) compose phi)(u)$. Вектор-функция $overline(gamma)^*$ непрерывно дифференцируема на отрезке $[a_1, b_1]$, и на нем производная этой функции не обращается в $(0, 0)$. Тогда вектор-функцию $overline(gamma)^*$ можно считать другим параметрическим представлением кривой $L$.

#definition()[
  Пусть $L_overline(gamma)$, где функция $overline(gamma)$ определена на $[a, b]$, и $L_(overline(gamma)^*)$, где функция $overline(gamma)^*$ определена на $[a_1, b_1]$ --- две параметризованные гладкие кривые. Говорят, что они эквиваленты, если существует функция $phi$ определенная на отрезке $[a_1, b_1]$, отображающая его на отрезок $[a, b]$, имеющая непрерывную производную с условием $forall u in [a_1, b_1] #h(6pt) phi'(u) != 0$, такая, что $overline(gamma)^* = (overline(gamma) compose phi)$.
]
...
#definition()[
  Пусть $L_overline(gamma)$, где функция $overline(gamma)$ определена на $[a, b]$, и $L_(overline(gamma)^*)$, где функция $overline(gamma)^*$ определена на $[a_1, b_1]$ --- две параметризованные гладкие кривые. Говорят, что они положительно эквиваленты, если существует функция $phi$ определенная на отрезке $[a_1, b_1]$, отображающая его на отрезок $[a, b]$, имеющая непрерывную производную с условием $forall u in [a_1, b_1] #h(6pt) phi'(u) > 0$, такая, что $overline(gamma)^* = (overline(gamma) compose phi)$.
]
