...
#definition(breakline: true)[
  Пусть функция $f$ дифференцируема в точке $overline(x)_0$. Определим функцию $d f(overline(x)_0): RR^m -> RR$, называемую дифференциалом функции $f$ в точке $overline(x)_0$:
...
#definition(breakline: true)[
  Пусть функция $f$ $n$-дифференцируема в точке $overline(x)_0$. Её $n$-дифференциалом в этой точке называется функция $d^n f(overline(x)_0): RR^m -> RR$ такая, что 
