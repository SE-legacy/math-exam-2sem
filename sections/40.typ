...
#definition()[
  Пусть функция $f$ дифференцируема в точке $overline(x)_0$. Для любого $overline(h) in RR^m$ определим функцию $d f(overline(x)_0): RR^m -> RR$, называемую дифференциалом функции $f$ в точке $overline(x)_0$:
  $ d f(overline(x)_0) (overline(h)) = sum^m_(k=1) (partial f)/(partial x_k) (overline(x)_0) h_k. $

  Если $phi(overline(x)_0) = x_k$, то обозначим её дифференциал как $d x_k$, тогда $d x_k (overline(h)) = h_k$,

  $ d f(overline(x)_0) = sum^m_(k=1) (partial f)/(partial x_k) d x_k. $
]
...
$ d^n f(overline(x)_0) = (d x_1 partial/(partial x_1) + dots + d x_m partial/(partial x_m))^n f(overline(x)_0). $
