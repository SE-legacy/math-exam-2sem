#import "../conf.typ": block_base, consequence, theorem
...
#theorem(name: "Теорема (аддитивность интеграла)")[
  ...
  Необходимость. $f in Re[a, c]$ и $f in Re[c, b], #h(6pt) epsilon > 0$. Тогда найдутся такое $P' = P'_([a, c])$ и $P'' = P''_([c, b])$, что
...
#theorem(name: "Теорема (монотонность интеграла)")[
  $f, g in Re[a, b], quad forall x in [a, b] #h(6pt) f(x) <= g(x)$. Тогда
...
#consequence(name: "Следствие 1")[
  $f in Re[a, b], #h(6pt) forall x #h(6pt) f(x) >= 0$. Тогда $limits(integral)_a^b f(x) d x >= 0$.
][]
...
#consequence(name: "Следствие 2")[
  ...
  $
    abs(limits(integral)_a^b f(x) d x) <= limits(integral)_a^b abs(f(x)) d x <= limits(sup)_(x in[a, b]) abs(f(x)) (b - a).
  $
][]
...
#theorem(name: "Теорема (первая теорема о среднем)")[
  ...
  $M = limits(sup)_(x in[a, b]) f(x), m = limits(inf)_(x in[a, b]) f(x)$. Тогда
  ...
  $forall x in [a, b] #h(6pt) g(x) >= 0, #h(6pt) m <= f(x) <= M$. Тогда
...
#consequence(breakline: true)[
  Если $g(x) equiv 1$, то $limits(integral)_a^b f(x) d x = f(xi)(b - a)$.

  Если $f$ непрерывна на $[a, b]$, то $exists xi in [a, b] quad limits(integral)_a^b f(x) d x = f(xi) (b - a)$.
][]
...
#theorem(name: "Теорема (операции над интегрируемыми функциями)")[
  ...
  + Докажем, что $f^(2)$ интегрируема:
    ...
    Тогда по критерию интегрируемости $f^(2) in Re[a, b]$.

    Т.к. $f dot g = (1)/(4)((f + g)^(2) - (f - g)^(2))$, то $f dot g in Re[a, b]$.
