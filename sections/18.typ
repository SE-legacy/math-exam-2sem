...
#theorem(
  ...
  $B_n = limits(sum)^n_(k=1) b_k$, последовательность $(a_n B_n)$ сходится. Тогда $sum a_n B_n$ и $sum B_n (a_(n+1) - a_n)$ ведут себя одинаково.
][]

#theorem(name: "Теорема (признак Дирихле)")[
  Последовательность $(a_n)$ монотонна и бесконечно малая; $B_n = limits(sum)^n_(k=1) b_k = O(1)$. Тогда ряд $sum a_n b_n$ сходится.
][
  ...
  Тогда по критерию Коши ряд $sum B_n (a_(n+1) - a_n)$ сходится.
]
