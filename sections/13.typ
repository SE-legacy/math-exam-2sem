...
#theorem(
  ...
  Ряды $sum a_n$, $sum b_n$ сходятся, и $limits(sum)^infinity_(n=1) a_n = A, #h(6pt) limits(sum)^infinity_(n=1) b_n = B, #h(6pt) lambda in RR$. Тогда ряды $sum(a_n + b_n), sum lambda a_n$ сходятся, причем 
