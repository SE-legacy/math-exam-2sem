#import "../conf.typ": definition, equ, theorem
...
#theorem(name: "Теорема (формула Грина для трапеции первого рода)")[
  ...
  #grid(columns: 2)[
    ...
    где функции $phi_1$, $phi_2$ непрерывно дифференцируемы на $[a,b]$. Обозначим точки $A(a, phi_1(a))$, $B(b, phi_1(b))$, $E(b, phi_2(b))$, $F(a, phi_2(a))$. Положительная ориентация границы $L$ соответствует последовательности точек $A B E F A$ (@fig:trapezoid).
  ][#trapezoid]
...
#theorem(name: "Теорема (формула Грина для трапеции второго рода)")[
  Пусть замкнутое множество $D$ является трапецией второго рода, $L$ --- положительно ориентированная граница $D$, а функция $Q$ и её частная производная $(partial Q) / (partial x)$ непрерывны на $D$. Тогда

  #equ(
    $ limits(integral.cont)_L Q(x,y) d y = limits(integral.double)_D (partial Q) / (partial x) (x,y) d x d y. $,
    id: <eq:greens-formula2>,
  )
][
...
#theorem(name: "Теорема (формула Грина)")[
  ...
  Пользуясь свойством аддитивности двойного интеграла, заменим интеграл по множеству $D$ суммой интегралов по частичным трапециям, на которых мы разобьём $D$.

  К интегралу по каждой трапеции применим формулу $(#ref(<eq:greens-formula1>))$ или $(#ref(<eq:greens-formula2>))$. Интегралы по общим участкам границ будут суммироваться дважды, причем с противоположными ориентациями, поэтому они взаимно уничтожатся. А сумма оставшихся составит криволинейный интеграл по границе множества $D$. Складывая формулы $(#ref(<eq:greens-formula1>))$ и $(#ref(<eq:greens-formula2>))$, придем к формуле из условия теоремы.
]
