...
#theorem(name: "Теорема (о дифференцируемости предела)")[
  Пусть $forall n in NN$ функция $f_n$ непрерывно дифференцируема на $[a, b]$, последовательность $(f'_n)$ равномерно сходится на $[a, b]$ к функции $phi$, последовательность $(f_n)$ сходится в точке $x_0 in X$.
