...
#definition()[
  Пусть вектор-функция $overline(f) = (f_1, dots, f_k)$ определена на отрезке $[a, b]$. Функция $overline(f)$ называется интегрируемой на отрезке $[a, b]$, если ее координатные функции $f_1, dots, f_k$ интегрируемы на этом отрезке. При этом полагают
