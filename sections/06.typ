...
#theorem(
  ...
  $f in Re[a, b]$, тогда $F(x) = limits(integral)_a^x f(t) d t$ непрерывна на $[a, b]$.
][
  Пусть $x, x + h in [a, b], #h(4pt) M = limits(sup)_(x in [a, b]) abs(f(x))$, тогда 
  ...
  при $h -> 0$. Тогда $F(x + h) -> F(x)$ при $h -> 0$, значит $forall x in [a, b] #h(4pt) F$ непрерывна в точке $x$.
]
