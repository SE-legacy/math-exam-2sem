...
#theorem(name: "Теорема (признак Дирихле)")[ 
  Пусть фукнции $f$, $g$, $g'$ непрерывны на $[a, omega)$, функция $F(b) = limits(integral)_a^b f(x) d x$ ограничена на $[a, omega)$, $g(x) -> 0$, монотонно убывая, при $x -> omega ==>$ несобсвенный интеграл $limits(integral)_a^omega f(x)g(x) d x$ сходится.
][
  $limits(lim)_(b -> omega) g(b)F(b) = 0$. Т.к. $g'(x) <= 0$, то 
  $ lim_(b -> omega) limits(integral)_a^b |g'(x)| d x = -lim_(b -> omega) limits(integral)_a^b g'(x) d x = -lim_(b -> omega) [g(b) - g(a)] = g(a), $ т.е. несобсвенный интеграл $limits(integral)_a^omega |g'(x)| d x$ сходится. Т.к. функция $F$ ограничена, то по признаку мажорации $limits(integral)_a^omega |g'(x)F(x)| d x$ сходится. Значит, сходится и $limits(integral)_a^omega g'(x) F(x) d x$. Осталось воспользоваться предыдущим утверждением.
]
