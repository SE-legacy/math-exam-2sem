...
#theorem(name: "Теорема (признак Дирихле)")[ 
  ...
  $ lim_(b -> omega) limits(integral)_a^b |g'(x)| d x = -lim_(b -> omega) limits(integral)_a^b g'(x) d x = -lim_(b -> omega) [g(b) - g(a)] = g(a), $ т.е. несобственный интеграл $limits(integral)_a^omega |g'(x)| d x$ сходится. Т.к. функция $F$ ограничена, то по признаку мажорации $limits(integral)_a^omega |g'(x)F(x)| d x$ сходится. Значит, сходится и $limits(integral)_a^omega g'(x) F(x) d x$. Осталось воспользоваться предыдущим утверждением.
]

#theorem(name: "Теорема (признак Абеля)")[ 
  Пусть функции $f$, $g$, $g'$ непрерывны на $[a, omega)$, несобственный интеграл $limits(integral)_a^omega f(x) d x$ сходится, $g$ монотонна и ограничена на $[a, omega) ==>$  несобственный интеграл $limits(integral)_a^omega f(x)g(x) d x$ сходится.
][
