...
#def 
$f$ опр. и огр. на $[a,b] quad M_k = limits(sup)_(x in Delta_k_n)f(x) quad m_k = limits(inf)_(x in Delta_k_n)f(x), k = overline(1\, n)$. 
$ S(P) = sum^n_(k=1)M_k Delta x_x, quad
s(P) = sum^n_(k=1)m_k Delta x_x $
наз. верхней и нижней суммами Дарбу соотв-нно функции $f$ для разбиения $P$.

#properties
+ $forall P #h(4pt) forall xi_P quad s(P)<= sigma(P,xi_P) <=S(P)$. \ #proof $forall xi_k in Delta_k quad m_k <= f(xi_k) <= M_k ==> limits(sum)^n_(k=1) m_k Delta x_k <= sigma(P, xi_P) <= limits(sum)^n_(k=1) M_k Delta x_k$. #qed

+ $P subset P_1 ==> s(P)<=s(P_1) quad S(P)>=S(P_1)$. \ #proof Пусть $P_1 = P union {x^*}, x^* in (x_(k-1), x_k);M'_k = limits(sup)_(x in [x_(k-1), x^*])f(x), M''_k = limits(sup)_(x in Delta [x^*, x_k])f(x) ==>$ для $S(P_1)$ слагаемое $M_k Delta x_k$ заменится на $M'_k (x^* - x_(k-1)) + M''_k (x_k - x^*) stretch(=>)_"монот. верх. гр." M'_k (x^* - x_(k-1)) + M''_k (x_k - x^*) <= M_k ((x^* - x_(k-1)) + (x_k - x^*)) = M_k Delta x_k ==> S(P_1) <= S(P)$. \ Для $s(P)$ аналог. #qed
...
#lemma() 
$P = P_([a,b]) quad d= d(P) quad P^*$ получается из $P$ добавлением $l$ точек. \
...
Пусть $f != "const" ==> M = limits(sup)_(x in [a,b])f(x) > m=limits(inf)_(x in [a,b]) f(x) quad epsilon > 0 ==> exists P^*: S(P)-overline(I) < epsilon/2$.

$l$ --- число точек $P^*$, не совп. с $a$ и $b quad P$ --- произв., $d(P)<delta=epsilon/(2l(M-m)) quad P'= P union P^* ==>$.

По пред. лемме $0<=S(P)-S(P')<=(M-m)l d < e/2 quad P^* subset P' ==>$
