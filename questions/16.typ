...
*Док-во*:
+ $alpha < q < 1 stretch(=>)^"опр."_"верх. пред." exists n_0 #h(4pt) forall n >= n_0 #h(4pt) root(n, a_n) <= limits(sup)_(n>= n_0) root(n, a_n) < q ==> a_n < q_n$. $sum q^n$ при $0< q<1$ сх-ся $stretch(=>)^"сл-е из"_"пр. маж." sum a_n$ сх-ся.
...
+ $alpha = 1 ==>$ вопрос о сх-ти открытый.
*Док-во*: 
+ $alpha < q < 1 stretch(=>)^"порядковые"_"св-ва пред." exists n_0 #h(4pt) forall n >= n_0 #h(4pt) (a_(n+1))/(a_n) < q = (q^(n+1))/(q^n)$. $sum q^n$ при $0< q<1$ сх-ся $stretch(=>)^"сл-е из"_"пр. маж."sum a_n$ сх-ся. \
