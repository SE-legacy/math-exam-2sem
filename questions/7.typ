...
#proof По пред. Т. $Phi$ имеет вид $Phi(x) = limits(integral)_(a)^(x) f(t) d t + C$, где $C = "const"$. Пусть сначала $x = a$, а потом $x = b$. Тогда $Phi(a) = limits(integral)_(a)^(a) f(t) d t + C = 0 + C = C$, $Phi(b) = limits(integral)_(a)^(b) f(t) d t + C ==> limits(integral)_(a)^(b) f(x) d x = Phi(b) - Phi(a)$. #qed
