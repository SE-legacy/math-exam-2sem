...
По индукции можно установить: $d^n f(overline(x)_0) = limits(sum)^m_(i_1, dots, i_n = 1) (diff^n f)/(diff x_i_1 dots diff x_i_n)(overline(x)_0) d x_i_1 dots d x_i_n.$ \
В силу независимости смешанных производных от порядка дифференцирования: $d^n f(overline(x)_0) = limits(sum)_(alpha_1 + dots + alpha_m = n) (n!)/(alpha_1 ! dots alpha_m !) (diff^n f)/(diff x_1^alpha_1 dots diff x_1^alpha_m)(overline(x)_0) d x_1^alpha_1 dots d x_1^alpha_m.$ \
Что символически можно записать: $d^n f(overline(x)_0) = (d x_1 diff/(diff x_1) + dots + d x_m diff/(diff x_m))^n f(overline(x)_0)$