...
*Док-во*: По следствию из пр. мажорации из сх-ти $sum b_n$ вытекает сходимость $sum a_n$. $limits(lim)_(n->infinity) (b_n)/(a_n) = 1/k stretch(=>)^"сл-е из"_"пр. маж."$ из сх-ти $sum a_n$ вытекает сходимость $sum b_n$.
