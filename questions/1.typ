...
#def 
$[a,b] subset RR quad P = P_([a,b]) = {x_k}^n_(k=0)$ наз. разбиением отрезка $[a,b]$.

$Delta k = [x_k, x_(k-1)] quad Delta x_k = x_k - x_(k-1) quad k = overline(1\, n)$.
...
Обозн. $I = limits(integral)^a_b f(x)d x quad f$ называют интегрируемой по Риману на $[a,b]$

#theorem(name: "необх. усл. интегрируемости") 
$f$ интегрируема на $[a,b] ==> f$ ограничена \
#proof 
От противного. Пусть $f$ не огр. на $[a,b] quad P$ --- произв. разбиение $==>$ $exists delta_i: f$ не огр. на нем.
...
forall M>0 #h(4pt) exists xi_i in Delta_i: quad abs(f(xi_i)) > (abs(A) + M)/(Delta x_i) ==> abs(sigma(P))>M ==> $
