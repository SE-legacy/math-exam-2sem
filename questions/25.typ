...
#theorem(name: "признак Дирихле равн. сх. функ. ряда") Пусть $forall x in X$ $f_n (x) arrow.b quad norm(limits(sum)_(k = n+1)^(n+p) g_k) = O(1) quad norm(f_n) = o(1)$.
Тогда ряд $sum f_n g_n$ равн. сх. \ 
#proof Пусть $G_n = limits(sum)_(k = 1)^(n) g_k quad G_(n,k) = limits(sum)_(i = 1)^(k) g_(n+i)$. 
...
Ряд $sum f_n g_n$ равн. сх. по крит. Коши. #qed
...
#proof Пусть $G_n = limits(sum)_(k = 1)^(n) g_k quad G_(n,k) = limits(sum)_(i = 1)^(k) g_(n+i) quad M > 0$ такое, что $forall n in NN$ $norm(f_n <= M)$, и $epsilon > 0$. 
Согласно крит. Коши равн. сх. функ. ряда: $exists n_(epsilon) in NN$ $forall n >= n_(epsilon)$ $forall k in NN$ $forall x in X$ 
$abs(limits(sum)_(k=n+1)^(n+p) f_k (x) g_k (x)) <= 2M norm(f_(n+1)) < 2M epsilon$. Ряд $sum f_n g_n$ равн. сх. по крит. Коши. #qed
