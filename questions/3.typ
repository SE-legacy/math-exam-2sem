...
$3==>1$. Пусть $underline(I) = overline(I) = A stretch(=>)^"осн. лемма"_"Дарбу" forall epsilon>0 #h(4pt) exists delta>0 #h(4pt) forall P #h(4pt) (d(P)< delta => (S(P)-A < epsilon and A-s(P) < epsilon)) stretch(=>)^"св-во 1"_("сумм Дарбу") A-epsilon<s(P)<= sigma(P,xi_P)<= S(P) < A + epsilon ==> abs(sigma(P, xi_P)-A) < epsilon$, т. е. $limits(lim)_(d->0) sigma(P, xi_P) = A ==> f$ интегрируема, $limits(integral)^b_a f(x) d x = A = underline(I) = overline(I)$. #qed