...
#def Функ. ряд вида $a_0 + a_1 (x-x_0) + a_2 (x-x_0)^2 + dots = limits(sum)_(n=0)^(infinity) a_n (x-x_0)^n$,
где $a_0, a_1, dots in RR quad x_0 in RR$, называют степ. рядом. 
Числа $a_0, a_1, dots$ назыв. коэфф. степ. ряда

#def Пусть $L = overline(limits(lim)_(n -> infinity)) root(n, abs(a_n))$. Определим число $R$:
