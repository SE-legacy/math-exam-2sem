...
*Т. (об арифм. д-ях над сх-ся рядами)*: $sum a_n, sum b_n$ сх-ся, и $limits(sum)^infinity_(n=1) a_n = A, limits(sum)^infinity_(n=1) b_n = B, lambda in RR ==> sum(a_n + b_n), sum lambda a_n$ сх-ся $limits(sum)^infinity_(n=1)(a_n + b_n) = A+B, limits(sum)^infinity_(n=1) lambda a_n = lambda A$ \
